...
!!! warning "Always enable TLS"
    Serve the API over HTTPS in production. The `auth_token` cookie is set with `Secure` + `HttpOnly`, so it is never sent over plain HTTP—but browsers will still send API key headers, and mixed setups invite downgrade or interception risks. Keep everything on HTTPS end-to-end.
